...
pub(super) const SESSION_IDLE_TIMEOUT: Duration = Duration::from_secs(600);
...
pub(super) struct SessionRef<'a> {
    pub(super) id: Uuid,
    pub(super) scope: &'a mut Scope<'static>,
}
...
impl SessionStore {
    ...
    fn with_limits(idle_timeout: Duration, max_sessions: usize) -> Self {
        Self {
    ...
    pub(super) fn reap(&mut self, now: Instant) -> usize {
