...
//!
//! Raised from [`GraphUI::prepass`](super::GraphUI::prepass) beside the pointer
...
pub(super) fn emit(ui: &mut Ui, doc: &Document, out: &mut Intents) {
    ...
    if duplicate && let Some(intent) = build_duplicate_intent(doc) {
        out.push(intent);
    }
    if delete {
