...
//!
//! **The curve.** A data connection ([`crate::gui::canvas::connection_ui`]) and
//! an event wire ([`crate::gui::canvas::subscription_ui`]) are both one
...
pub(super) struct GlyphDrag<A, B> {
...
impl<A: GlyphKey, B: GlyphKey> GlyphDrag<A, B> {
    ...
    pub(super) fn latch(layer: &PortLayer<A>, keys: impl Iterator<Item = A>) -> Option<Self> {
    ...
    pub(super) fn held(self, layer: &PortLayer<A>) -> bool {
...
impl WireTint {
    ...
    pub(super) fn new(start: Color, end: Color) -> Self {
        Self { start, end }
    }
    ...
    /// color.
    pub(super) fn flat(color: Color) -> Self {
        Self {
            start: color,
            end: color,
        }
