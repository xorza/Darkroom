...
/// widget ids. Rebuilt in [`crate::gui::pane::graph::GraphUI::prepass`] and
/// reused by [`crate::gui::pane::graph::GraphUI::draw`]. Each glyph snapshot is sized to the four
...
impl<K: GlyphKey> PortLayer<K> {
    ...
    /// (`crate::gui::pane::graph::node::port_row`); everything else that reads a center is
    /// inside the canvas.
...
impl CanvasGeometry {
    ...
    pub(crate) fn node_screen_rect(&self, node_id: NodeId) -> Option<Rect> {
    ...
    pub(crate) fn over_any_node(&self, pointer: Vec2) -> bool {
