...
use crate::stacking::star_detection::detector::FwhmSource;
...
///
/// Precedence: auto-estimation (if enabled) wins, falling back to `expected` (else a default) when
/// too few stars are found; otherwise the fixed `expected` is used. A [`FwhmSource::Configured`]
/// answer is one this frame did not measure — a preset's `expected`, or that fallback — and
/// [`FwhmSource::Estimated`] carries the count of stars that actually produced the value.
pub(crate) fn estimate(
    ...
    // Auto-estimation takes precedence; `expected` becomes its fallback when too few stars are
    // found (see `from_stars`).
    if config.fwhm.auto_estimate {
        return from_bright_stars(pixels, stats, config, pool);
    }

    // Otherwise use the fixed expected FWHM (0 disables the matched filter).
    if config.fwhm.expected > f32::EPSILON {
        return FwhmSource::Configured(config.fwhm.expected);
    }
...
fn from_bright_stars(
    ...
    let regions = DetectResult::from_image(pixels, stats, None, &first_pass_config, pool).regions;
    tracing::debug!(
    ...
    from_stars(&stars, &config.fwhm, &config.filter)
}
...
fn from_stars(
    ...
    if fwhms.len() < min_stars {
        ...
        // `fallback_fwhm` has no dependence on `fwhms` — it is `expected` or the hardcoded
        // default — so this is `Configured`, not an `Estimated` that never measured anything.
        return FwhmSource::Configured(fallback_fwhm);
    }
