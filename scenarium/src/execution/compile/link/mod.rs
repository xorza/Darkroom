//! Linking: one [`LoweredGraph`] becomes one [`CompiledGraph`].
//!
//! This is where the crate's two identity spaces meet. Lowering works in stable
//! [`NodeId`]s because that is all a walk over authored graphs can
...
//! crosses over, and it crosses in one direction: it takes the lowered graph
...
use crate::execution::compiled::{ExecutionBinding, ExecutionEvent, ExecutionInput, ExecutionNode};
...
/// [`Lowerer`](crate::execution::lower::Lowerer), for the same
...
pub(super) struct Linker {
    /// Positions into the lowered node list, sorted by id — the order that becomes
    /// the dense index space. An index sort rather than sorting the nodes, since
    /// the lowered graph is read-only here.
    order: Vec<u32>,
...
impl Linker {
    /// Link `lowered` into `out`: place its nodes in the dense index space, resolve
    ...
    ///
    /// **Reads the lowered graph, writes the artifact.** `lowered` is shared, so
    /// linking copies what it needs (the lambdas it copies are `Arc`s) and leaves
    /// the graph exactly as lowering left it: the same one links twice to the same
    ...
    pub(super) fn link(
        ...
        assert!(
            u32::try_from(lowered.e_nodes.len()).is_ok(),
            "program node count must fit in u32"
        );

        self.order.clear();
        self.order.extend(0..lowered.e_nodes.len() as u32);
        self.order
            .sort_unstable_by_key(|&at| lowered.node_ids[at as usize]);

        let mut node_ids = Column::default();
        let mut node_index = HashMap::with_capacity(lowered.e_nodes.len());
        let mut e_nodes = Column::default();
        for (position, &at) in self.order.iter().enumerate() {
            let at = at as usize;
            let node_id = lowered.node_ids[at];
            let previous = node_index.insert(node_id, NodeIdx(position as u32));
            ...
            // moving is what leaves the lowered graph readable — the lambda is an
            // `Arc`, so it costs a refcount bump.
            e_nodes.push(lowered.e_nodes[at].clone());
        }

        let inputs = intern_bindings(&lowered.inputs, &node_index);
        ...
        outputs.append(lowered.outputs.iter().cloned());
        let events = self.wire_subscriptions(
            library,
            lowered.events,
            &lowered.subscriptions,
            &e_nodes,
            &node_index,
        );
    ...
    ///
    /// Both endpoints exist for the same reason a bind target does: lowering
    ...
    /// event. So a miss here is a lowering bug, and panics rather than silently
    ...
    fn wire_subscriptions(
        ...
        for subscription in subscriptions {
            let subscriber_idx = *node_index
                .get(&subscription.subscriber)
                .expect("lowering only subscribes nodes it emitted");
            let emitter_idx = *node_index
                .get(&subscription.event.node_id)
                .expect("lowering only subscribes to emitters it emitted");
            let events = e_nodes[emitter_idx].events;
            assert!(
                subscription.event.event_idx < events.len as usize,
                "lowering only subscribes to events the emitter declares"
            );
...
///
/// Every target exists: lowering only binds producers it emitted, so a miss is a
/// lowering bug rather than bad input, and degrading it to an unbound input
...
fn intern_bindings(
    lowered: &Column<InputIdx, LoweredInput>,
    node_index: &HashMap<NodeId, NodeIdx>,
) -> Column<InputIdx, ExecutionInput> {
    let mut inputs = Column::default();
    inputs.append(lowered.iter().map(|input| ExecutionInput {
        required: input.required,
        stamps_fs_path: input.stamps_fs_path,
        binding: match &input.binding {
            LoweredBinding::None => ExecutionBinding::None,
            // The one payload a link copies rather than moves — the price of the
            // lowered graph staying readable afterwards.
            LoweredBinding::Const(value) => ExecutionBinding::Const(value.clone()),
            LoweredBinding::Bind(producer) => {
                let node_idx = *node_index
                    .get(&producer.node_id)
                    .expect("lowering only binds producers it emitted");
